...
Editor::Editor()
    : highlighter(UserConfig::get().getPath("syntax/knight.json").string()),
  ...
  for (int i = 0; i < config.screenRows; i++) {
    ...
    if (fileRow >= rows.size()) {
      ...
      if (rows.size() == 0 && i == config.screenRows / 3) {
        std::string greeting =
            "Scribe Editor -- version " + std::string(SCRIBE_VERSION);
...
void Editor::applyHighlighting(Row *row, HighlightTheme theme,
                               std::string *ab) {
  int len = row->getSize();
  for (int j = 0; j < len; j++) {
...
void Editor::drawStatusBar(std::string *ab) {
  ...
  std::string status = filename.substr(0, 20) + " - " +
  ...
  if (availableSpace >=
      static_cast<int>(std::to_string(config.y + 1).length())) {
...
static void findCallback(std::vector<Row> rows, Config &config,
...
std::string Editor::prompt(std::string prompt,
                           void (*callback)(std::vector<Row>, Config &config,
                                            std::string, int)) {
  std::string input;
...
void Editor::save() {
  ...
  if (!outputFile.is_open()) {
    statusMessage =
        "Can't save! I/O error: " + std::string(std::strerror(errno));
    return;
  }
