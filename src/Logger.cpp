...
std::string Logger::currentDateTime()
{
    ...
    oss << std::put_time(std::localtime(&time), "%Y-%m-%d %H:%M:%S") << '.' << std::setw(3) << std::setfill('0')
        << ms.count();
    return oss.str();
}
...
std::string Logger::formatMessage(Level level, const std::string& message, const char* file, const char* function,
                                  int line)
{
    std::string levelStr;
    switch (level)
    ...
    std::ostringstream oss;
    oss << "[" << currentDateTime() << "] [" << levelStr << "] [" << shortFile << ":" << line << "@"
